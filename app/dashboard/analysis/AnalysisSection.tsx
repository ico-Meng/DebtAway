...
export default function AnalysisSection({ 
  ...
  // Tracks whether any of the 3 inputs (job, scope, resume) changed since the last successful analysis run
  // Starts true so button is enabled on fresh mount; set to false after analysis succeeds
  const [inputsChangedSinceLastAnalysis, setInputsChangedSinceLastAnalysis] = useState<boolean>(true);
  ...
  const [showPersonalCapabilityPrompt, setShowPersonalCapabilityPrompt] = useState<boolean>(false);
  ...
  useEffect(() => {
    try {
      ...
      // Read cachedNavigationId once here and reuse below
      const cachedNavIdStr = localStorage.getItem('cachedNavigationId');
      const cachedNavId = cachedNavIdStr ? parseInt(cachedNavIdStr, 10) : undefined;

      // When the user clicks the analysis icon from "From existing resume", always run a fresh
      // analysis — never show stale cached results. Detect this by checking that we are in
      // existing-resume mode with a new navigationId that differs from what was last cached.
      const isFreshFromExistingResume =
        fromExistingResume &&
        autoTriggerAnalysis &&
        initialNavigationId !== undefined &&
        initialNavigationId !== cachedNavId;

      if (isFreshFromExistingResume) {
        // Wipe stale cached analysis from localStorage so nothing lingers after this mount
        try {
          localStorage.removeItem('cachedPersonalCapabilityAnalysis');
          localStorage.removeItem('cachedResumePowerAnalysis');
          localStorage.removeItem('cachedSelectedAnalysisType');
          localStorage.removeItem('cachedShowPersonalCapabilityPrompt');
        } catch { /* ignore */ }
      } else if (cachedPersonalCapability || cachedResumePower) {
        // Normal case: restore cached analysis results (tab switch / page refresh)
        ...
        // Restore personal capability prompt flag (persists the centered-dot + message state across navigation)
        const cachedPrompt = localStorage.getItem('cachedShowPersonalCapabilityPrompt');
        if (cachedPrompt === 'true') {
          setShowPersonalCapabilityPrompt(true);
        }
      }
      ...
      // If cached analysis results exist (and this is NOT a fresh existing-resume navigation),
      // restore the previousInitialDataRef sentinel so the auto-trigger effect treats this as
      // "already analyzed" rather than "new data on fresh mount".
      // navigationId is included so a fresh click (new ID) still triggers a new analysis even
      // when the job title/company is unchanged.
      const cachedJobDataRaw = localStorage.getItem('cachedJobDataAnalysis');
      if (!isFreshFromExistingResume && cachedJobDataRaw && (cachedPersonalCapability || cachedResumePower)) {
        try {
          const cachedJobData = JSON.parse(cachedJobDataRaw);
          previousInitialDataRef.current = {
            fetchedJobData: cachedJobData,
            resumeFile: null,
            navigationId: cachedNavId,
          };
          hasAutoTriggeredRef.current = true;
        } catch {
          // Ignore parse errors — auto-trigger will handle fresh state
        }
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    // navigationId is the primary signal: each click of the analysis icon stamps a fresh
    // Date.now(), so even the same job triggers a new analysis. Value comparison of
    // job title/company is a fallback for cases where navigationId is absent.
    // resumeFile is only compared when the ref holds a real File (not a null sentinel).
    const hasNewInitialData =
      ...
      (
        ...
        // New click from resume page — navigationId changed
        (initialNavigationId !== undefined &&
          initialNavigationId !== previousInitialDataRef.current.navigationId) ||
        // Fallback: job changed (value comparison handles remount sentinel case)
        previousInitialDataRef.current.fetchedJobData?.target_job_title !== initialFetchedJobData.target_job_title ||
        previousInitialDataRef.current.fetchedJobData?.target_job_company !== initialFetchedJobData.target_job_company ||
        (previousInitialDataRef.current.resumeFile !== null &&
    ...
    if (hasNewInitialData) {
      ...
      // New data arrived from resume section — re-enable the button
      setInputsChangedSinceLastAnalysis(true);
      // Clear cached analysis results when new data arrives from resume page
      setPersonalCapabilityAnalysis(null);
      setResumePowerAnalysis(null);
      setSelectedDimension(null);
      // Reset prompt — will be set to true by runAnalysis once backend responds
      setShowPersonalCapabilityPrompt(false);
      // Also clear from localStorage
      try {
        localStorage.removeItem('cachedPersonalCapabilityAnalysis');
        localStorage.removeItem('cachedResumePowerAnalysis');
        localStorage.setItem('cachedShowPersonalCapabilityPrompt', 'false');
        if (initialNavigationId !== undefined) {
          localStorage.setItem('cachedNavigationId', String(initialNavigationId));
        }
      } catch (error) {
    ...
    // When fromExistingResume, bypass knowledge scope requirement (resume-only analysis)
    if (
      fetchedJobData &&
      analysisResumeFile &&
      user?.profile?.sub &&
      (fromExistingResume || analysisKnowledgeScope.establishedExpertise || analysisKnowledgeScope.expandingKnowledgeBase) &&
      !isAnalyzing
    ) {
  ...
  // Compute whether all 3 required inputs are present (job verified, scope selected, resume displayed)
  // When fromExistingResume, knowledge scope is not required (resume-only analysis)
  const allInputsPresent =
    fetchedJobData !== null &&
    (fromExistingResume || analysisKnowledgeScope.establishedExpertise || analysisKnowledgeScope.expandingKnowledgeBase) &&
    (analysisResumeFile !== null || cachedResumeFileName !== null);

  // "Analyzed" state: all inputs present but no input has changed since the last successful analysis run
  const isAnalyzedState = allInputsPresent && !inputsChangedSinceLastAnalysis;
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      if (result.status === 'success') {
        ...
        // Mark that analysis has been run and inputs are now in sync — button goes "unclickable"
        setInputsChangedSinceLastAnalysis(false);

        if (fromExistingResume) {
          // Resume-only mode: ignore any personal capability response from the backend
          setPersonalCapabilityAnalysis(null);
          setSelectedAnalysisType('resume');
          setShowPersonalCapabilityPrompt(true);
        } else if (!result.personal_capability) {
          // No DynamoDB knowledge data found: default to resume view and prompt user to build profile/knowledge
          setPersonalCapabilityAnalysis(null);
          setSelectedAnalysisType('resume');
          setShowPersonalCapabilityPrompt(true);
          // Uncheck both knowledge scope checkboxes to reflect missing data
          const emptyScope = { establishedExpertise: false, expandingKnowledgeBase: false };
          setAnalysisKnowledgeScope(emptyScope);
          try { localStorage.setItem('cachedAnalysisKnowledgeScope', JSON.stringify(emptyScope)); } catch {}
        } else {
        ...
        try {
          localStorage.setItem('cachedPersonalCapabilityAnalysis', fromExistingResume ? 'null' : JSON.stringify(result.personal_capability));
          localStorage.setItem('cachedResumePowerAnalysis', JSON.stringify(result.resume_power));
          localStorage.setItem('cachedSelectedAnalysisType', fromExistingResume ? 'resume' : 'personal');
          localStorage.setItem('cachedAnalysisKnowledgeScope', JSON.stringify(analysisKnowledgeScope));
          localStorage.setItem('cachedShowPersonalCapabilityPrompt', (fromExistingResume || !result.personal_capability) ? 'true' : 'false');
          if (initialNavigationId !== undefined) {
            localStorage.setItem('cachedNavigationId', String(initialNavigationId));
          }
          if (analysisResumeFile) {
        ...
        <div className={styles.analysisLeftColumn}>
          <div className={`${styles.analysisLeftUpper} ${isAnalysisResultsFadingOut ? styles.analysisResultsFadeOut : ''}`}>
            <HexGraph
              ...
              showPersonalDotOnly={showPersonalCapabilityPrompt && selectedAnalysisType === 'personal'}
                ...
                >
                  {isAnalyzing ? (
                    ...
                    <>
                      ...
                      <span className={styles.nextButtonText}>Analyzed</span>
                    </>
                  ) : (
        ...
        <div className={`${styles.analysisRightContent} ${isAnalyzing ? styles.analysisRightContentCentered : ''}`}>
            ...
            <div style={{ 
              ...
              <button
                ...
                onClick={() => {
                  ...
                  // When from existing resume with no personal data, show prompt and uncheck scopes
                  if (showPersonalCapabilityPrompt) {
                    const emptyScope = { establishedExpertise: false, expandingKnowledgeBase: false };
                    setAnalysisKnowledgeScope(emptyScope);
                    try { localStorage.setItem('cachedAnalysisKnowledgeScope', JSON.stringify(emptyScope)); } catch {}
                  }
          ...
          {isAnalyzing ? (
            ...
            <div className={`${styles.analysisCardsContainer} ${isAnalysisResultsFadingOut ? styles.analysisResultsFadeOut : ''}`} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {selectedAnalysisType === 'personal' && showPersonalCapabilityPrompt && (
                <div className={`${styles.analysisInfoCard} ${styles.personalCapabilityCard}`} style={{ cursor: 'default' }}>
                  <p className={styles.analysisInfoText}>
                    <span className={styles.analysisTypeLabel}>Personal Capability:</span>{' '}
                    Build your profile and knowledge base, then check the knowledge scope checkboxes and re-run analysis to evaluate your personal capability for the target role.
                  </p>
                  <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1.5rem', flexWrap: 'wrap' }}>
                    <button
                      type="button"
                      onClick={onNavigateToProfile}
                      className={styles.buildProfileButton}
                    >
                      Build Profile
                    </button>
                    <button
                      ...
                      className={styles.buildKnowledgeButton}
                    >
                      Build Knowledge
                    </button>
                  </div>
