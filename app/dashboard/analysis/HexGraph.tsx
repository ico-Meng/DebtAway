...
export default function HexGraph({
  ...
  useEffect(() => {
    ...
    // When showing personal dot only (no DynamoDB established expertise data)
    if (showPersonalDotOnly && selectedType === 'personal') {
      // Draw only a central dot to indicate personal capability data is unavailable
      g.append('circle')
        .attr('cx', 0)
        .attr('cy', 0)
        .attr('r', 7)
      ...
      // Also draw resume hexagon behind if available
      if (resumePowerScores) {
        drawDataHexagon(g, resumePowerScores, 'resume', labels, angleSlice, radius, defs, false, 0.15);
      }
      return;
    }
