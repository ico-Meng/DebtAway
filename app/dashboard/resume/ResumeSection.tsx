...
interface ResumeSectionProps {
  ...
  /** Resume file from profile page to pre-load in "From existing resume" panel */
  initialResumeFileForExisting?: File | null;
}
...
export default function ResumeSection({
  ...
  const [hasCachedResumeKB, setHasCachedResumeKB] = useState<boolean>(false);
  const [hasCachedResumeExisting, setHasCachedResumeExisting] = useState<boolean>(false);
  const pendingCacheSaveRef = useRef<'kb' | 'existing' | null>(null);
  ...
  const [showJobTooltipAutoFromExistingResume, setShowJobTooltipAutoFromExistingResume] = useState<boolean>(false);
  ...
  // Separate crafting state for "From Existing Resume" panel — independent from knowledge-base flow
  const [isExistingResumeCrafting, setIsExistingResumeCrafting] = useState<boolean>(false);
  const [existingResumeCraftingCardIndex, setExistingResumeCraftingCardIndex] = useState<number>(0);
  ...
  const existingResumeJobTooltipHoverTimerRef = useRef<NodeJS.Timeout | null>(null);
  const [isExistingResumeTooltipHovered, setIsExistingResumeTooltipHovered] = useState<boolean>(false);
  const existingResumeJobTooltipRef = useRef<HTMLDivElement>(null);
  ...
  const RESUME_CACHE_KB_KEY = 'resumeDocCache_kb';
  const RESUME_CACHE_EXISTING_KEY = 'resumeDocCache_existing';
  ...
  const saveResumeState = () => {
    ...
    try {
      ...
      // Save resume file metadata only when a file is active.
      // Do NOT remove the key when resumeFile is null — deletion is handled
      // explicitly in the remove-file button so the cached name survives page refreshes.
      if (resumeFile) {
        const fileMetadata = {
  ...
  const loadResumeState = () => {
    try {
      ...
      if (savedState) {
        ...
        if (parsed.showCompanyTypePage !== undefined && !showCompanyTypePage) setShowCompanyTypePage(parsed.showCompanyTypePage);
        if (parsed.showExistingResumePage !== undefined && !showExistingResumePage) setShowExistingResumePage(parsed.showExistingResumePage);
      ...
      // But we can restore the file name for display purposes
      const fileMetadata = localStorage.getItem(RESUME_FILE_KEY);
      if (fileMetadata) {
        try {
          const parsed = JSON.parse(fileMetadata);
          if (parsed.name) setStoredResumeFileName(parsed.name);
        } catch {}
        setResumeFile(null);
      }

      // Initialize cached resume flags from presence of separate cache keys
      setHasCachedResumeKB(localStorage.getItem(RESUME_CACHE_KB_KEY) !== null);
      setHasCachedResumeExisting(localStorage.getItem(RESUME_CACHE_EXISTING_KEY) !== null);
    } catch (error) {
      console.error('Error loading resume state from localStorage:', error);
    }
  };
  
  // Pre-load resume file from profile page when "From existing resume" page opens
  useEffect(() => {
    if (showExistingResumePage && initialResumeFileForExisting && !resumeFile) {
      setResumeFile(initialResumeFileForExisting);
    }
  ...
  // Keep storedResumeFileName in sync with the current resumeFile
  useEffect(() => {
    if (resumeFile) {
      setStoredResumeFileName(resumeFile.name);
    }
  }, [resumeFile]);

  // Re-derive jobInputTypeFromExistingResume from restored prop value on mount
  // (after page refresh the prop may have a cached value but jobInputTypeFromExistingResume defaults to null)
  // Also auto-show the tooltip if fetchedJobDataFromExistingResume is already populated (restored from cache)
  useEffect(() => {
    if (interestedJobPositionFromExistingResume) {
      setJobInputTypeFromExistingResume(detectJobInputType(interestedJobPositionFromExistingResume));
    }
    if (fetchedJobDataFromExistingResume) {
      setShowJobTooltipAutoFromExistingResume(true);
      jobTooltipTimerRef.current = setTimeout(() => {
        setShowJobTooltipAutoFromExistingResume(false);
      }, 3000);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  // Helper: load a mode-specific cached resume doc and navigate to resume page
  const loadCachedResumeDoc = (mode: 'kb' | 'existing') => {
    try {
      const cacheKey = mode === 'kb' ? RESUME_CACHE_KB_KEY : RESUME_CACHE_EXISTING_KEY;
      const cached = localStorage.getItem(cacheKey);
      if (!cached) return;
      const data = JSON.parse(cached);
      if (data.savedName !== undefined) setSavedName(data.savedName);
      if (data.savedContactFields !== undefined) setSavedContactFields(data.savedContactFields);
      if (data.savedProfessionalExperiences !== undefined) setSavedProfessionalExperiences(data.savedProfessionalExperiences);
      if (data.savedEducation !== undefined) setSavedEducation(data.savedEducation);
      if (data.savedProjectsEstablished !== undefined) setSavedProjectsEstablished(data.savedProjectsEstablished);
      if (data.savedProjectsExpanding !== undefined) setSavedProjectsExpanding(data.savedProjectsExpanding);
      if (data.savedSkills !== undefined) setSavedSkills(data.savedSkills);
      if (data.savedAchievements !== undefined) setSavedAchievements(data.savedAchievements);
      if (data.resumeMode !== undefined) setResumeMode(data.resumeMode);
      if (data.industrySector !== undefined) setIndustrySector(data.industrySector);
      if (data.targetJobPosition !== undefined) setTargetJobPosition(data.targetJobPosition);
      if (data.storedResumeFileName !== undefined) setStoredResumeFileName(data.storedResumeFileName);
    } catch (error) {
      console.error('Error loading cached resume doc:', error);
    }
    if (mode === 'kb') {
  ...
  // Save to mode-specific cache when pendingCacheSaveRef is set and saved* state updates
  useEffect(() => {
    if (!pendingCacheSaveRef.current) return;
    const mode = pendingCacheSaveRef.current;
    pendingCacheSaveRef.current = null;
    const cacheKey = mode === 'kb' ? RESUME_CACHE_KB_KEY : RESUME_CACHE_EXISTING_KEY;
    try {
      localStorage.setItem(cacheKey, JSON.stringify({
        ...
        resumeMode: mode === 'kb' ? 'industry' : 'existing',
        ...(mode === 'existing' && storedResumeFileName ? { storedResumeFileName } : {}),
      }));
    } catch (error) {
      console.error('Error saving cached resume doc:', error);
    }
    if (mode === 'kb') setHasCachedResumeKB(true);
    else setHasCachedResumeExisting(true);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // Card-switching timer for "From Existing Resume" crafting — completely independent
  useEffect(() => {
    if (!isExistingResumeCrafting) return;
    if (existingResumeCraftingCardIndex >= 5) return;

    const intervalId = setInterval(() => {
      setExistingResumeCraftingCardIndex((prev) => (prev >= 5 ? 5 : prev + 1));
    }, 7000);

    return () => clearInterval(intervalId);
  }, [isExistingResumeCrafting, existingResumeCraftingCardIndex]);
  ...
  const handleExistingResumeNext = async () => {
    ...
    try {
      // Build structured job position payload
      let targetJobPositionData: {
        title: string;
        company: string;
        description: string;
      ...
      if (fetchedJobDataFromExistingResume) {
        targetJobPositionData = {
          title: fetchedJobDataFromExistingResume.target_job_title,
          company: fetchedJobDataFromExistingResume.target_job_company,
          description: fetchedJobDataFromExistingResume.target_job_description,
          skill_keywords: fetchedJobDataFromExistingResume.target_job_skill_keywords,
        };
      } else if (interestedJobPositionFromExistingResume) {
        // User typed a job title or pasted a description without fetching
        targetJobPositionData = interestedJobPositionFromExistingResume;
      }
      ...
      formData.append('form_data', JSON.stringify(metaPayload));

      const response = await fetch(`${API_ENDPOINT}/craft_resume_from_existing_resume`, {
        method: 'POST',
        body: formData,
      });
      ...
      if (result.success && result.data) {
        ...
        console.error('Failed to craft resume from existing file:', result.message);
        alert(`Failed to craft resume: ${result.message || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  const handleJobPositionChangeFromExistingResume = (value: string) => {
    ...
    setJobInputTypeFromExistingResume(inputType);
    ...
    if (value.trim()) {
      ...
      setJobInputTypeFromExistingResume(null);
  ...
  const handleFetchJobUrlFromExistingResume = async () => {
    ...
    const currentInputType = detectJobInputType(interestedJobPositionFromExistingResume);
    ...
    try {
      ...
      if (currentInputType === 'url') {
        ...
        response = await fetch(`${API_ENDPOINT}/fetch_with_job_title`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ job_title: interestedJobPositionFromExistingResume.trim() })
        });
        result = await response.json();
        ...
        if (!validateJobDescriptionContent(interestedJobPositionFromExistingResume)) {
        ...
        response = await fetch(`${API_ENDPOINT}/parse_job_description`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ job_description: interestedJobPositionFromExistingResume.trim() })
        });
        result = await response.json();
  ...
  const generateResumePDF = async (): Promise<{ blob: Blob; filename: string } | 'DOWNLOAD_LIMIT_EXCEEDED' | null> => {
    try {
      ...
      // Check if the response is JSON (quota exceeded) vs PDF blob
      ...
      if (contentType.includes('application/json')) {
        ...
        console.error('Unexpected JSON response from PDF endpoint:', result);
        return null;
      }
  ...
  const handleNavigateToAnalysisWithData = async () => {
    ...
    if (resumeMode === 'existing') {
      ...
      // If no structured job data but the user typed a job position, construct a minimal
      // FetchedJobData so the analysis auto-trigger condition (requires non-null fetchedJobData)
      // is satisfied and the backend receives the user's intent.
      if (!fetchedJobData && interestedJobPositionFromExistingResume.trim()) {
        const inputText = interestedJobPositionFromExistingResume.trim();
        const inputType = detectJobInputType(inputText);
        fetchedJobData = {
          target_job_title: inputType !== 'job_description' ? inputText : '',
          target_job_company: '',
          target_job_description: inputType === 'job_description' ? inputText : '',
          target_job_skill_keywords: [],
        };
      }

      // Prefer the fetched job title for a clean display in the analysis input,
      // falling back to the raw user input (URL / job title text)
    ...
    // When coming from "From existing resume", uncheck both scopes — only resume power analysis runs
    const isExistingResumeMode = resumeMode === 'existing';
    const currentKnowledgeScope = isExistingResumeMode
      ? { establishedExpertise: false, expandingKnowledgeBase: false }
      : { establishedExpertise: knowledgeScope.establishedExpertise, expandingKnowledgeBase: knowledgeScope.expandingKnowledgeBase };
  ...
  const handleResumeBack = () => {
    // Save current saved* state to the mode-specific cache
    const cacheKey = resumeMode === 'existing' ? RESUME_CACHE_EXISTING_KEY : RESUME_CACHE_KB_KEY;
    try {
      localStorage.setItem(cacheKey, JSON.stringify({
        savedName, savedContactFields, savedProfessionalExperiences, savedEducation,
        ...
        industrySector,
        targetJobPosition,
        ...(resumeMode === 'existing' && storedResumeFileName ? { storedResumeFileName } : {}),
      }));
    } catch (error) {
      console.error('Error saving cached resume doc on back:', error);
    }
    if (resumeMode === 'existing') setHasCachedResumeExisting(true);
    else setHasCachedResumeKB(true);
    ...
    if (resumeMode === 'existing') {
      ...
      // Keep both fields — don't clear either, so the user's latest input is preserved
      if (industrySector) {
        setInterestedCompanyType(industrySector);
        // Do not clear interestedJobPositionFromExistingResume — keep the user's typed text
      } else if (targetJobPosition) {
        ...
        <div className={styles.resumeSectionHeader}>
          <button
            ...
            onClick={() => {
              ...
              setResumeMode(null);
              setInterestedCompanyType('');
              // Keep resumeFile and interestedJobPosition cached
            }}
            aria-label="Back"
          >
            <svg
          ...
          {hasCachedResumeExisting && (
            <button
              type="button"
              className={`${styles.backButton} ${styles.resumeTopBackButton}`}
              onClick={() => loadCachedResumeDoc('existing')}
            ...
            <div className={styles.formField}>
              ...
              {jobUrlFetchFailed ? (
                ...
                <div
                  ...
                  onMouseEnter={() => {
                    if (fetchedJobDataFromExistingResume) {
                      if (existingResumeJobTooltipHoverTimerRef.current) {
                        clearTimeout(existingResumeJobTooltipHoverTimerRef.current);
                        existingResumeJobTooltipHoverTimerRef.current = null;
                      }
                      setIsExistingResumeTooltipHovered(true);
                    }
                  }}
                  onMouseLeave={() => {
                    if (fetchedJobDataFromExistingResume) {
                      existingResumeJobTooltipHoverTimerRef.current = setTimeout(() => {
                        setIsExistingResumeTooltipHovered(false);
                        existingResumeJobTooltipHoverTimerRef.current = null;
                      }, 300);
                    }
                  }}
                  ...
                  {(isJobUrlValidFromExistingResume || fetchedJobDataFromExistingResume || isJobUrlBlockedFromExistingResume) && !isCheckmarkFadingOut && (
                    <>
                      {isJobUrlBlockedFromExistingResume && blockedMessage && (
                        <div className={`${styles.jobUrlFetchTooltip} ${showJobTooltipAutoFromExistingResume ? styles.jobUrlFetchTooltipVisible : ''}`}>
                      ...
                      {fetchedJobDataFromExistingResume && (
                        <div
                          ref={existingResumeJobTooltipRef}
                          className={`${styles.jobUrlFetchTooltip} ${(showJobTooltipAutoFromExistingResume || isExistingResumeTooltipHovered) ? styles.jobUrlFetchTooltipVisible : ''}`}
                          onMouseEnter={() => {
                            if (existingResumeJobTooltipHoverTimerRef.current) {
                              clearTimeout(existingResumeJobTooltipHoverTimerRef.current);
                              existingResumeJobTooltipHoverTimerRef.current = null;
                        ...
                        >
                          ...
                          <div className={styles.jobUrlFetchTooltipSkills}>
                            {fetchedJobDataFromExistingResume.target_job_skill_keywords.map((skill, index) => (
                ...
                <div className={styles.resumeUploadArea}>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setResumeFile(file);
                      if (file) {
                        // Persist metadata immediately so it survives page refresh
                        try {
                          localStorage.setItem(RESUME_FILE_KEY, JSON.stringify({
                    ...
                    {(resumeFile || storedResumeFileName) && (
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          e.stopPropagation();
                          setResumeFile(null);
                          setStoredResumeFileName(null);
                          try { localStorage.removeItem(RESUME_FILE_KEY); } catch {}
            ...
            {isExistingResumeCrafting && (
              <div className={styles.craftingProgressText}>
                {[
                  ...
                  'Enriching your projects and skills...',
                  ...
                  <span
                    ...
                    className={`${styles.craftingProgressTextItem} ${
                      index === existingResumeCraftingCardIndex ? styles.craftingProgressTextItemActive : ''
                    }`}
                  >
                    {index === existingResumeCraftingCardIndex && text.split('').map((char, charIndex) => (
                      <span
                        key={charIndex}
                    ...
                    {index !== existingResumeCraftingCardIndex && text}
                  </span>
                ))}
              </div>
            )}
        ...
        <div className={styles.resumeSectionHeader}>
          ...
          {hasCachedResumeKB && (
            <button
              type="button"
              className={`${styles.backButton} ${styles.resumeTopBackButton}`}
              onClick={() => loadCachedResumeDoc('kb')}
              aria-label="Go to resume"
            >
              <svg
                className={styles.backButtonIcon}
  ...
  if (showResumePage) {
    const activeJobData = resumeMode === 'existing'
      ? fetchedJobDataFromExistingResume
            ...
            <div className={styles.resumeLeftColumnContent}>
              {industrySector ? (
                <>
                  ...
                  {(resumeMode === 'existing' ? interestedJobPositionFromExistingResume : interestedJobPositionFromKnowledgeBase) && (
                    <div className={`${styles.resumeLeftField} ${styles.resumeLeftFieldCompact}`}>
                      <label className={styles.resumeLeftLabel} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }} onMouseEnter={() => { if (jobPosHideTimer.current) clearTimeout(jobPosHideTimer.current); if (jobPosShowTimer.current) clearTimeout(jobPosShowTimer.current); jobPosShowTimer.current = setTimeout(() => setHoveredJobPosLabel('display'), 600); }} onMouseLeave={() => { if (jobPosShowTimer.current) { clearTimeout(jobPosShowTimer.current); jobPosShowTimer.current = null; } setHoveredJobPosLabel(null); }}>
                        <span>Interested Job Position</span>
                      ...
                      <div className={styles.resumeLeftDisplayText}>
                        {activeJobData ? (
                          <>
                            <div className={styles.resumeLeftJobTitle}>{activeJobData.target_job_title}</div>
                            <div className={styles.resumeLeftJobCompany}>{activeJobData.target_job_company}</div>
                          </>
                        ) : (
                ...
                <div className={styles.resumeLeftField}>
                  <label className={styles.resumeLeftLabel}>
                    {resumeMode === 'existing' ? 'Interested Job Position' : 'Target Job Position'}
                  </label>
                  <div className={styles.resumeLeftDisplayText}>
                    {activeJobData ? (
                      <>
                ...
                <div className={styles.resumeLeftField}>
                  <label className={styles.resumeLeftLabel} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }} onMouseEnter={() => { if (industrySectorLabelHideTimer.current) clearTimeout(industrySectorLabelHideTimer.current); if (industrySectorLabelShowTimer.current) clearTimeout(industrySectorLabelShowTimer.current); industrySectorLabelShowTimer.current = setTimeout(() => setHoveredIndustrySectorLabelKey('display-dropdown'), 600); }} onMouseLeave={() => { if (industrySectorLabelShowTimer.current) { clearTimeout(industrySectorLabelShowTimer.current); industrySectorLabelShowTimer.current = null; } setHoveredIndustrySectorLabelKey(null); }}>
                    <span>Industry Sector</span>
                ...
                <div className={styles.resumeLeftField}>
                  ...
                  <div className={styles.resumeLeftUploadContainer}>
                    <div className={styles.resumeLeftUploadArea}>
                      ...
                      />
                      <label htmlFor="resume-left-upload" className={styles.resumeLeftUploadLabel} style={{ pointerEvents: 'none', cursor: 'default' }}>
