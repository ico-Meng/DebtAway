...
// Bullet-line helpers: techAndTeamwork uses trailing-'\n' format for N lines
// '' = 0 lines; 'a\n' = 1 line 'a'; 'a\nb\n' = 2 lines.  Backward-compat with old 'a\nb' (no trailing \n).
const getBulletLines = (techAndTeamwork: string): string[] => {
  if (!techAndTeamwork) return [];
  return techAndTeamwork.endsWith('\n')
    ? techAndTeamwork.split('\n').slice(0, -1)
    : techAndTeamwork.split('\n');
};
const updateBulletLine = (techAndTeamwork: string, index: number, value: string): string => {
  const lines = getBulletLines(techAndTeamwork);
  lines[index] = value;
  return lines.length === 0 ? '' : lines.join('\n') + '\n';
};
const addBulletLine = (techAndTeamwork: string): string => techAndTeamwork + '\n';
const removeBulletLine = (techAndTeamwork: string, index: number): string => {
  const lines = getBulletLines(techAndTeamwork);
  lines.splice(index, 1);
  return lines.length === 0 ? '' : lines.join('\n') + '\n';
};

// IndexedDB helpers for resume file persistence across page refreshes
const RESUME_IDB_DB = 'dashboard-resume-cache';
const RESUME_IDB_STORE = 'resume';

async function saveResumeToIDB(file: File): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    req.onsuccess = (e) => {
      ...
      tx.objectStore(RESUME_IDB_STORE).put(file, 'current');
...
async function loadResumeFromIDB(): Promise<File | null> {
  return new Promise((resolve) => {
    const req = indexedDB.open(RESUME_IDB_DB, 1);
    req.onupgradeneeded = (e) => (e.target as IDBOpenDBRequest).result.createObjectStore(RESUME_IDB_STORE);
    req.onsuccess = (e) => {
      const db = (e.target as IDBOpenDBRequest).result;
      const tx = db.transaction(RESUME_IDB_STORE, 'readonly');
      const getReq = tx.objectStore(RESUME_IDB_STORE).get('current');
      getReq.onsuccess = () => { db.close(); resolve((getReq.result as File) ?? null); };
...
type AutoFillApiResponse = {
  ...
  email: string; phone: string; addressStreet: string; addressState: string; addressZip: string;
  personalWebsite: string; linkedin: string;
  education: Array<{ collegeName: string; location: string; degrees: Array<{ degree: string; major: string; startMonth: string; startYear: string; endMonth: string; endYear: string; coursework: string; }> }>;
  companies: Array<{ companyName: string; jobTitle: string; startMonth: string; startYear: string; endMonth: string; endYear: string; isPresent: boolean; location: string; }>;
  achievements: Array<{ type: string; value: string; }>;
  personal_projects: Array<{ projectName: string; projectDescription: { overview: string; techAndTeamwork: string; achievement: string; }; projectStartMonth: string; projectStartYear: string; projectEndMonth: string; projectEndYear: string; location: string; selectedTechnologies: string[]; selectedFrameworks: string[]; customTechnologies: string[]; customFrameworks: string[]; }>;
  professional_projects: Array<{ projectName: string; projectDescription: { overview: string; techAndTeamwork: string; achievement: string; }; selectedWorkExperience: string; projectStartMonth: string; projectStartYear: string; projectEndMonth: string; projectEndYear: string; selectedTechnologies: string[]; selectedFrameworks: string[]; customTechnologies: string[]; customFrameworks: string[]; }>;
  technical_skill_items: string[];
  custom_technical_items: string[];
};
...
export default function DashboardPage() {
  ...
  // Check whether any profile/knowledge data already exists
  const hasExistingData = () =>
    !!(careerFocus || firstName || lastName || colleges.length || professionalExperiences.length ||
       personalProjects.length || professionalProjects.length || selectedTechnicalSkills.length);

  // Apply data returned from the auto-fill endpoint into all form state
  const applyAutoFillData = (data: AutoFillApiResponse) => {
    // Career focus
    setCareerFocus(data.career_focus || 'software-engineering');
    ...
    // Education — set companies first so professional projects can reference them
    setProfessionalExperiences(data.companies.map((c) => ({
      id: crypto.randomUUID(), ...c,
    })));

    setColleges(data.education.map((edu) => ({
      id: crypto.randomUUID(),
      collegeName: edu.collegeName,
      location: edu.location,
      degrees: edu.degrees.map((deg) => ({ id: crypto.randomUUID(), ...deg })),
    })));
    ...
    // Collect custom keywords from all projects
    const newCustomKeywords: Record<string, string[]> = {};
    const newCustomFrameworkKeywords: Record<string, string[]> = {};

    const mappedPersonal = data.personal_projects.map((p) => {
      if (p.customTechnologies.length > 0)
        newCustomKeywords['Other'] = [...(newCustomKeywords['Other'] ?? []), ...p.customTechnologies];
      if (p.customFrameworks.length > 0)
        newCustomFrameworkKeywords['Other'] = [...(newCustomFrameworkKeywords['Other'] ?? []), ...p.customFrameworks];
      return {
        id: crypto.randomUUID(),
        projectName: p.projectName,
        projectDescription: p.projectDescription,
        selectedIndustries: [],
        projectStartMonth: p.projectStartMonth,
        projectStartYear: p.projectStartYear,
        projectEndMonth: p.projectEndMonth,
        projectEndYear: p.projectEndYear,
        location: p.location,
        selectedTechnologies: p.selectedTechnologies,
        selectedFrameworks: p.selectedFrameworks,
        isInterviewReady: false,
      };
    });
    setPersonalProjects(mappedPersonal);

    const mappedProfessional = data.professional_projects.map((p) => {
      if (p.customTechnologies.length > 0)
        newCustomKeywords['Other'] = [...(newCustomKeywords['Other'] ?? []), ...p.customTechnologies];
      if (p.customFrameworks.length > 0)
        newCustomFrameworkKeywords['Other'] = [...(newCustomFrameworkKeywords['Other'] ?? []), ...p.customFrameworks];
      return {
        ...
        projectName: p.projectName,
        projectDescription: p.projectDescription,
        selectedWorkExperience: p.selectedWorkExperience,
        projectStartMonth: p.projectStartMonth,
        projectStartYear: p.projectStartYear,
        projectEndMonth: p.projectEndMonth,
        projectEndYear: p.projectEndYear,
    ...
    setProfessionalProjects(mappedProfessional);

    // Merge custom keywords (deduplicate)
    setCustomKeywords((prev) => {
      const merged = { ...prev };
      for (const [k, v] of Object.entries(newCustomKeywords)) {
    ...
    setCustomFrameworkKeywords((prev) => {
      const merged = { ...prev };
      for (const [k, v] of Object.entries(newCustomFrameworkKeywords)) {
        merged[k] = Array.from(new Set((merged[k] ?? []).concat(v)));
      }
      return merged;
    });

    // Technical Skill Focus — union of predefined items, add custom ones
    setSelectedTechnicalSkills((prev) => Array.from(new Set(prev.concat(data.technical_skill_items))));
    if (data.custom_technical_items.length > 0) {
      setCustomTechnicalSkillKeywords((prev) => ({
        ...prev,
        'Other': Array.from(new Set((prev['Other'] ?? []).concat(data.custom_technical_items))),
      }));
    }

    // Navigate to Career Focus and show completion message
  ...
  const handleAutoFill = async () => {
    ...
    try {
      ...
      formData.append('cognito_sub', user?.profile?.sub ?? '');

      const res = await fetch(`${API_ENDPOINT}/auto_fill_info_from_resume`, {
        method: 'POST',
        body: formData,
      });
  ...
  const handleKnowledgeSubmit = async () => {
    ...
    try {
      const knowledgeData = {
        ...
        custom_framework_keywords: customFrameworkKeywords,
      };
  ...
  const fetchKnowledge = async () => {
    ...
    try {
      const response = await fetch(`${API_ENDPOINT}/get_knowledge/${encodeURIComponent(user.profile.sub)}?career_focus=${encodeURIComponent(careerFocus)}`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      if (result.status === 'success' && result.knowledge_exists && result.data) {
        ...
        // Restore custom technology/framework keyword options per section
        if (knowledgeData.custom_keywords && typeof knowledgeData.custom_keywords === 'object') {
          setCustomKeywords(knowledgeData.custom_keywords);
        }
        if (knowledgeData.custom_framework_keywords && typeof knowledgeData.custom_framework_keywords === 'object') {
          setCustomFrameworkKeywords(knowledgeData.custom_framework_keywords);
        }
  ...
  const handleExpandingKnowledgeSubmit = async () => {
    ...
    try {
      const expandingKnowledgeData = {
        ...
        custom_future_keywords: customFutureKeywords,
        custom_future_framework_keywords: customFutureFrameworkKeywords,
      };
  ...
  const fetchExpandingKnowledge = async () => {
    ...
    try {
      const response = await fetch(`${API_ENDPOINT}/get_expanding_knowledge/${encodeURIComponent(user.profile.sub)}?career_focus=${encodeURIComponent(careerFocus)}`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      if (result.status === 'success' && result.expanding_knowledge_exists && result.data) {
        ...
        // Restore custom technology/framework keyword options per section
        if (expandingKnowledgeData.custom_future_keywords && typeof expandingKnowledgeData.custom_future_keywords === 'object') {
          setCustomFutureKeywords(expandingKnowledgeData.custom_future_keywords);
        }
        if (expandingKnowledgeData.custom_future_framework_keywords && typeof expandingKnowledgeData.custom_future_framework_keywords === 'object') {
          setCustomFutureFrameworkKeywords(expandingKnowledgeData.custom_future_framework_keywords);
        }
  ...
  // AI & Machine Learning specific technology sections (abstract concepts per layer)
  const aiMlTechnologySections = {
    'AI Application & Product': [
      'Prompt Engineering', 'RAG', 'AI Agents', 'Agentic Workflows', 'Multi-modal AI',
      'Conversational AI', 'Chain-of-Thought Reasoning', 'Few-shot Learning', 'Zero-shot Learning',
      'Function / Tool Calling', 'Semantic Search', 'Embeddings & Vector Representations',
      'AI Evaluation & Benchmarking', 'RLHF', 'Fine-tuning Strategies', 'In-context Learning',
      'Multi-agent Systems', 'Memory & State Management', 'Knowledge Graphs', 'Output Validation',
      'Token Optimization', 'Context Window Management', 'AI Personalization', 'Hybrid Search',
      'Task Decomposition', 'Self-reflection & Self-critique', 'AI Safety & Guardrails',
      'Instruction Following', 'Content Moderation', 'AI Ethics & Bias Mitigation', 'Other',
    ],
    'Foundation Model & AI Research': [
      'Transformer Architecture', 'Self-attention / Cross-attention', 'Pre-training Objectives',
      'Self-supervised Learning', 'Contrastive Learning', 'Mixture of Experts (MoE)',
      'Diffusion Models', 'Model Scaling Laws', 'Knowledge Distillation',
      'Quantization (INT4/INT8/BF16)', 'Parameter-Efficient Fine-Tuning (PEFT)', 'LoRA / QLoRA',
      'Instruction Tuning', 'DPO (Direct Preference Optimization)', 'Constitutional AI',
      'Mechanistic Interpretability', 'Multimodal Learning', 'Continual Learning', 'Meta-learning',
      'Sparse Attention', 'Neural Architecture Search', 'Model Alignment', 'Adversarial Robustness',
      'Emergent Capabilities', 'Positional Encoding (RoPE/ALiBi)', 'Tokenization Strategies',
      'Model Merging / Model Soup', 'Speculative Decoding', 'Preference Learning',
      'Retrieval-Augmented Pre-training', 'Other',
    ],
    'MLOps & ML Platform': [
      'CI/CD for ML', 'Feature Stores', 'Model Registry', 'Experiment Tracking',
      'Model Monitoring & Observability', 'Data Drift Detection', 'A/B Testing for ML',
      'Pipeline Orchestration', 'Shadow Deployment', 'Canary Deployment', 'Model Versioning',
      'Hyperparameter Optimization (HPO)', 'Inference Optimization', 'Model Serving Architecture',
      'Model Governance', 'Model Card Documentation', 'Responsible AI Frameworks',
      'Reproducibility', 'Multi-tenancy ML Platforms', 'Model Compression Pipelines',
      'Online vs. Batch Inference', 'Blue-Green Deployment', 'Resource Scheduling',
      'Cost Optimization for ML', 'End-to-end ML Workflow Design', 'AutoML',
      'Distributed Training Coordination', 'ML Metadata Management', 'Feedback Loops',
      'Data Labeling Pipelines', 'Other',
    ],
    'Data Engineering & Analytics': [
      'ETL / ELT Pipelines', 'Data Lakehouse Architecture', 'Stream Processing',
      'Batch Processing', 'Data Modeling (Star/Snowflake)', 'Data Quality & Validation',
      'Data Lineage', 'Feature Engineering', 'Data Versioning', 'Data Augmentation',
      'Synthetic Data Generation', 'Schema Evolution', 'Data Governance',
      'Distributed Query Processing', 'Column-oriented Storage', 'Vector Search & Indexing',
      'Embedding Pipelines', 'Semantic Data Modeling', 'Change Data Capture (CDC)',
      'Data Mesh Architecture', 'Real-time Analytics', 'Metadata Management', 'Data Catalog',
      'Data Contracts', 'Multi-modal Data Processing', 'Unstructured Data Processing',
      'Time-series Data Management', 'Approximate Nearest Neighbor (ANN)',
      'Data Freshness & SLAs', 'Federated Learning Data Patterns', 'Other',
    ],
    'Infrastructure & Compute': [
      'GPU Architecture (warp/SM)', 'CUDA Programming Model', 'Distributed Computing Paradigms',
      'Model Parallelism', 'Data Parallelism', 'Pipeline Parallelism', 'Tensor Parallelism',
      'Mixed Precision Training (FP16/BF16)', 'Gradient Checkpointing',
      'Memory Bandwidth Optimization', 'Kernel Fusion',
      'High-Performance Networking (InfiniBand)', 'Hardware Accelerator Design',
      'NPU / TPU Architecture', 'Compiler Optimization for ML', 'Operator Fusion',
      'Graph Optimization', 'Low-latency Inference', 'Batching Strategies',
      'KV Cache Optimization', 'Flash Attention Algorithm',
      'Collective Communication (AllReduce)', 'Preemptible Compute',
      'HPC Cluster Management', 'Power & Thermal Management',
      'Edge AI / On-device Inference', 'Sparsity & Pruning at HW Level',
      'Interconnect Topologies', 'NVLink Architecture', 'Heterogeneous Computing', 'Other',
    ],
  };
  ...
  const aiMlFrameworkSections = {
    'AI Application & Product': [
      ...
      'LangGraph', 'AutoGen', 'CrewAI', 'Streamlit', 'Gradio', 'FastAPI', 'Chainlit', 'DSPy',
      'Guardrails AI', 'RAGAS', 'LangSmith', 'Promptflow (Azure)', 'Semantic Kernel',
      'Haystack', 'Agno (PhiData)', 'Pinecone', 'Weaviate', 'Chroma', 'Qdrant',
      'GPT-4o', 'Claude 3.5 Sonnet', 'Gemini 1.5 Pro', 'Llama 3', 'Mistral Large',
      'Cohere API', 'Other',
    ],
    'Foundation Model & AI Research': [
      'PyTorch', 'TensorFlow', 'JAX / Flax', 'Hugging Face Transformers', 'PEFT Library',
      'TRL (Transformer RL)', 'DeepSpeed', 'Megatron-LM', 'Flash Attention', 'vLLM',
      'llama.cpp', 'Ollama', 'bitsandbytes', 'FSDP', 'torchtune', 'Axolotl', 'Unsloth',
      'LM Evaluation Harness', 'Weights & Biases (W&B)', 'ClearML',
      'Triton (OpenAI kernel)', 'ONNX', 'TensorRT', 'Whisper (ASR)', 'CLIP',
      'Stable Diffusion 3', 'LLaMA-Factory', 'SGLang', 'ExLlamaV2', 'LightEval', 'Other',
    ],
    'MLOps & ML Platform': [
      'MLflow', 'Kubeflow', 'Apache Airflow', 'Prefect', 'DVC (Data Version Control)',
      'BentoML', 'Seldon Core', 'Ray / Ray Tune / Ray Serve',
      'NVIDIA Triton Inference Server', 'AWS SageMaker', 'Google Vertex AI',
      'Azure Machine Learning', 'ZenML', 'Evidently AI', 'Argo Workflows', 'Feast',
      'Tecton', 'Label Studio', 'Scale AI', 'DataRobot', 'Docker', 'Kubernetes', 'Helm',
      'Prometheus / Grafana', 'Great Expectations', 'Optuna', 'W&B Sweeps',
      'Metaflow', 'CometML', 'Flyte', 'Other',
    ],
    'Data Engineering & Analytics': [
      'Apache Spark', 'Apache Kafka', 'dbt (Data Build Tool)', 'Snowflake', 'Databricks',
      'BigQuery', 'Amazon Redshift', 'Pinecone', 'Weaviate', 'Chroma', 'Qdrant', 'Milvus',
      'Pandas', 'Polars', 'DuckDB', 'Delta Lake', 'Apache Iceberg', 'Fivetran', 'Airbyte',
      'Apache Flink', 'Trino / Presto', 'Apache Hudi', 'LanceDB', 'pgvector',
      'Elasticsearch', 'MongoDB Atlas Search', 'dbt Cloud', 'Apache Arrow', 'Parquet',
      'Apache Beam', 'Other',
    ],
    'Infrastructure & Compute': [
      'CUDA / cuDNN', 'NCCL', 'MPI / OpenMPI', 'TensorRT', 'ONNX Runtime', 'OpenCL',
      'ROCm (AMD)', 'AWS Trainium / Inferentia', 'Google Cloud TPU', 'Intel Gaudi',
      'Slurm', 'Triton (OpenAI kernel lang)', 'XLA', 'NVIDIA A100 / H100',
      'Cerebras CS-3', 'AWS EC2 P4/P5 Instances', 'PyTorch FSDP', 'NVIDIA Nsight',
      'cuBLAS / cuSPARSE', 'Singularity / Apptainer', 'NVIDIA Apex',
  ...
  // AI & Machine Learning specific technical skill focus sections (recruitment-focused skills per layer)
  const aiMlTechnicalSkillFocusSections = {
    'AI Application & Product': [
      'Prompt Engineering', 'RAG', 'LangChain', 'AI Agents', 'LlamaIndex', 'Multi-modal AI',
      'OpenAI API', 'Fine-tuning', 'LangGraph', 'Vector Databases', 'Semantic Search',
      'Chain-of-Thought', 'AutoGen', 'Few/Zero-shot Learning', 'Streamlit',
      'Hugging Face Hub', 'FastAPI', 'Function Calling', 'AI Safety & Guardrails',
      'Embeddings', 'RLHF', 'CrewAI', 'Agentic Workflows',
      'LLM APIs (GPT/Claude/Gemini)', 'DSPy', 'Hybrid Search', 'AI Evaluation (RAGAS)',
      'Multi-agent Systems', 'LangSmith', 'Knowledge Graphs', 'Other',
    ],
    'Foundation Model & AI Research': [
      'PyTorch', 'Transformer Architecture', 'Fine-tuning / PEFT', 'LoRA / QLoRA',
      'Hugging Face Transformers', 'LLM Pre-training', 'Instruction Tuning', 'vLLM',
      'DeepSpeed', 'Flash Attention', 'Model Quantization', 'RLHF / DPO',
      'Weights & Biases', 'TRL', 'Mixture of Experts', 'Multimodal Learning', 'FSDP',
      'Ollama', 'bitsandbytes', 'Speculative Decoding', 'Mechanistic Interpretability',
      'Model Alignment', 'Axolotl', 'Scaling Laws', 'ONNX', 'llama.cpp',
      'Contrastive Learning', 'Diffusion Models', 'Knowledge Distillation', 'JAX / Flax', 'Other',
    ],
    'MLOps & ML Platform': [
      'MLflow', 'Kubeflow', 'AWS SageMaker', 'Apache Airflow', 'Ray / Ray Serve',
      'Docker + Kubernetes', 'CI/CD for ML', 'Model Monitoring', 'Feature Stores',
      'NVIDIA Triton', 'Vertex AI', 'Azure ML', 'DVC', 'BentoML', 'HPO', 'Prefect',
      'ZenML', 'Feast', 'Optuna', 'Great Expectations', 'Evidently AI', 'Label Studio',
      'AutoML', 'Model Versioning', 'Seldon Core', 'Argo Workflows',
      'Weights & Biases', 'Experiment Tracking', 'Data Drift Detection', 'Model Registry', 'Other',
    ],
    'Data Engineering & Analytics': [
      'Apache Spark', 'Apache Kafka', 'dbt', 'Snowflake', 'Databricks', 'BigQuery',
      'Vector Databases', 'Feature Engineering', 'ETL / ELT Pipelines', 'Data Lakehouse',
      'Delta Lake / Iceberg', 'Pandas / Polars', 'Stream Processing', 'Apache Flink',
      'DuckDB', 'ANN Search', 'CDC', 'Data Mesh', 'pgvector', 'Milvus',
      'Real-time Analytics', 'Synthetic Data Generation', 'Apache Arrow', 'Parquet',
      'Data Versioning', 'LanceDB', 'Trino / Presto', 'Embedding Pipelines',
      'Data Governance', 'Airbyte / Fivetran', 'Other',
    ],
    'Infrastructure & Compute': [
      'CUDA / cuDNN', 'Distributed Training (FSDP/DDP)', 'TensorRT', 'ONNX Runtime',
      'GPU Architecture', 'Mixed Precision Training', 'DeepSpeed ZeRO', 'Model Parallelism',
      'HuggingFace Accelerate', 'Slurm', 'AWS Trainium', 'Google Cloud TPU', 'NCCL',
      'Triton (kernel)', 'torch.compile', 'XLA', 'Low-latency Inference',
      'KV Cache Optimization', 'Kernel Fusion', 'Flash Attention (impl.)', 'Edge AI',
      'ROCm (AMD)', 'MPI', 'HPC Cluster Management', 'Ray Cluster', 'CUDA Graphs',
      'MIG (Multi-Instance GPU)', 'Collective Communication', 'NVIDIA H100/A100',
      'FP8 Training', 'Other',
    ],
  };
  ...
  const [showAutoFillSuccessMessage, setShowAutoFillSuccessMessage] = useState<boolean>(false);
  ...
  // Auto-hide "Saved!" message after 3 seconds (matches animation duration)
  useEffect(() => {
    if (showSavedMessage) {
  ...
  // Auto-hide auto-fill success message after 3 seconds
  useEffect(() => {
    if (showAutoFillSuccessMessage) {
      const timer = setTimeout(() => {
        setShowAutoFillSuccessMessage(false);
      }, 3000);
      return () => clearTimeout(timer);
    }
  }, [showAutoFillSuccessMessage]);

  // Auto-hide "Saved!" message for established expertise after 3 seconds (matches animation duration)
  useEffect(() => {
  ...
  // Auto-hide "Saved!" message for expanding knowledge base after 3 seconds (matches animation duration)
  useEffect(() => {
  ...
  const saveDashboardState = () => {
    ...
    try {
      const stateToSave = {
        ...
        analysisJobPosition,
        // Resume section — "From Existing Resume" field cache
        resumeInterestedJobPositionFromExistingResume,
        resumeFetchedJobDataFromExistingResume,
      };
  ...
  const loadDashboardState = () => {
    try {
      ...
      if (savedState) {
        ...
        // Restore "From Existing Resume" field cache
        if (parsed.resumeInterestedJobPositionFromExistingResume !== undefined)
          setResumeInterestedJobPositionFromExistingResume(parsed.resumeInterestedJobPositionFromExistingResume);
        if (parsed.resumeFetchedJobDataFromExistingResume !== undefined)
          setResumeFetchedJobDataFromExistingResume(parsed.resumeFetchedJobDataFromExistingResume);
      }
  ...
  // Load resume file from IndexedDB on mount (persists across page refreshes)
  useEffect(() => {
    loadResumeFromIDB().then((file) => {
  ...
  useEffect(() => {
    ...
    // Wait for the backend fetch to finish before initializing —
    // prevents creating an empty project that races with real fetched data
  ...
  }, [activeExpertiseStep, personalProjects.length, isKnowledgeLoading]);
  ...
  useEffect(() => {
    ...
    // Wait for the backend fetch to finish before initializing
    if (isExpandingKnowledgeLoading) {
      return;
    }
  ...
  }, [activeExpandingKnowledgeStep, futureProfessionalProjects.length, isExpandingKnowledgeLoading]);
  ...
  const handleLookUp = async (
    projectId: string,
    section: 'personal' | 'professional' | 'futurePersonal' | 'futureProfessional',
    sourceInput: string
  ) => {
    const isFuture = section === 'futureProfessional';
    const setLoading = isFuture ? setIsFutureLookUpLoading : setIsLookUpLoading;
    const setError = isFuture ? setFutureLookUpError : setLookUpError;
    const setMode = isFuture ? setFutureDescriptionMode : setDescriptionMode;

    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`${API_ENDPOINT}/get_bullet_points_from_project_source`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success) {
        const newDesc = {
          ...
          techAndTeamwork: data.data.techAndTeamwork || '',
          achievement: data.data.achievement || '',
        };
        if (section === 'personal') {
          markEstablishedDirty();
          setPersonalProjects(prev => prev.map(p => p.id === projectId ? { ...p, projectDescription: newDesc } : p));
        } else if (section === 'professional') {
          markEstablishedDirty();
          setProfessionalProjects(prev => prev.map(p => p.id === projectId ? { ...p, projectDescription: newDesc } : p));
        } else if (section === 'futurePersonal') {
          markExpandingDirty();
          setFuturePersonalProjects(prev => prev.map(p => p.id === projectId ? { ...p, projectDescription: newDesc } : p));
        } else {
          markExpandingDirty();
          setFutureProfessionalProjects(prev => prev.map(p => p.id === projectId ? { ...p, projectDescription: newDesc } : p));
        }
        setMode('bulletPoints');
      } else {
        setError(data.message || 'Failed to generate bullet points. Please try again.');
      }
    } catch {
  ...
  // Read ?tab= from URL on mount and set activeSection accordingly
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const tab = params.get('tab');
    if (tab === 'knowledge' || tab === 'resume' || tab === 'profile') {
      setActiveSection(tab);
    } else if (tab === 'analysis' || tab === 'analyzer') {
      setActiveSection('analyzer');
    }
  }, []);

  // Sync URL query param when activeSection changes
  useEffect(() => {
    const tabValue = activeSection === 'analyzer' ? 'analysis' : activeSection;
    router.replace(`/dashboard?tab=${tabValue}`, { scroll: false });
  }, [activeSection]);
  ...
  // Reset knowledge state when career focus changes so the correct focus data is re-fetched
  useEffect(() => {
    if (careerFocus && prevCareerFocusRef.current && prevCareerFocusRef.current !== careerFocus) {
      // Reset Established Expertise state
      setPersonalProjects([]);
      setProfessionalProjects([]);
      setSelectedTechnicalSkills([]);
      setCustomTechnicalSkillKeywords({});
      setCustomTechnicalSkillLayers([]);
      setCustomKeywords({});
      setCustomFrameworkKeywords({});
      setIsKnowledgeFetched(false);

      // Reset Expanding Knowledge Base state
      setFuturePersonalProjects([]);
      setFutureProfessionalProjects([]);
      setSelectedFutureTechnicalSkills([]);
      setCustomFutureTechnicalSkillKeywords({});
      setCustomFutureTechnicalSkillLayers([]);
      setCustomFutureKeywords({});
      setCustomFutureFrameworkKeywords({});
      setIsExpandingKnowledgeFetched(false);
    }
    prevCareerFocusRef.current = careerFocus;
  }, [careerFocus]);
  ...
  useEffect(() => {
    ...
    // 3. Established Expertise is being shown
    // 4. Knowledge hasn't been fetched yet
    // 5. Not currently loading
    if (user?.profile?.sub && careerFocus && showEstablishedExpertise && !isKnowledgeFetched && !isKnowledgeLoading) {
      fetchKnowledge();
    }
  ...
  if (isLoading) {
    return (
      <div className={`${orbitron.variable} ${comfortaa.variable}`} style={{
        minHeight: '100vh',
        display: 'flex',
        flexDirection: 'column',
        alignItems: 'center',
        justifyContent: 'center',
        background: 'linear-gradient(135deg, #edece3 0%, #f5f2eb 50%, #edece3 100%)',
        backgroundSize: '200% 200%',
        animation: 'loadingGradientShift 6s ease infinite',
      }}>
        <style>{`
        ...
        <div style={{
          ...
          overflow: 'hidden',
          marginBottom: '2.5rem',
          animation: 'loadingFadeIn 0.6s ease 0.1s both',
        }}>
          <div style={{
            height: '100%',
            width: '45%',
        ...
        <div style={{
          ...
          gap: '8px',
          marginBottom: '1.5rem',
          animation: 'loadingFadeIn 0.6s ease 0.2s both',
        }}>
          {[0, 1, 2].map(i => (
            <div key={i} style={{
              width: '8px',
              ...
              borderRadius: '50%',
              background: '#2d2d2d',
              animation: `loadingPulse 1.4s ease-in-out ${i * 0.16}s infinite`,
            }} />
          ))}
        </div>
        ...
        <p style={{
          ...
          margin: 0,
          animation: 'loadingFadeIn 0.6s ease 0.3s both',
        }}>
            ...
            <div className={styles.contentWrapper}>
                ...
                <>
                    ...
                    <div className={styles.resumeUploadArea}>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setResumeFile(file);
                          if (file) saveResumeToIDB(file).catch(console.error);
                        }}
                      />
                      <label htmlFor="resume-upload" className={styles.resumeUploadLabel}>
                        ...
                        <div className={styles.resumeUploadContent}>
                          <span className={styles.resumeUploadTitle}>
                            {resumeFile ? resumeFile.name : 'Upload resume for auto-fill'}
                          </span>
                  ...
                  <div className={styles.nextButtonContainer} style={{ marginTop: '3rem', justifyContent: 'center', gap: '15rem', width: '100%' }}>
                    <button
                      ...
                      disabled={isAutoFillLoading || !resumeFile}
                      aria-label="Auto fill profile from resume"
                    >
                      {isAutoFillLoading ? (
                        <span className={styles.backButtonText}>Filling...</span>
                      ) : (
                        <span className={styles.backButtonText}>Auto fill</span>
                      )}
                    </button>
                    <button
                      type="button"
              ...
              {activeSection === 'knowledge' && showEstablishedExpertise && (
                <>
                  ...
                  <div className={`${styles.profileStepsContainer} ${styles.knowledgeStepsContainer}`}>
                    <div className={styles.progressBarContainer}>
                      <div 
                        className={styles.progressBar}
                        style={{
                  ...
                  <div className={styles.profilePanel}>
                    ...
                    {activeExpertiseStep === 'Personal Project' && (
                      <div className={styles.knowledgePanelSection}>
                        ...
                        {isKnowledgeLoading ? (
                          <div className={styles.collegeSection} style={{ alignItems: 'center', justifyContent: 'center', padding: '3rem 2rem' }}>
                              ...
                              <div className={styles.knowledgeFormRow} style={{ marginBottom: '1.5rem' }}>
                                <div className={styles.knowledgeFormField} style={{ maxWidth: '300px', minWidth: '200px' }}>
                                  <label htmlFor={`project-name-${project.id}`} className={styles.formLabel}>
                                    Project Name <span className={styles.requiredIndicator}>*</span>
                                  </label>
                                  <input
                                    type="text"
                                ...
                                <div className={styles.knowledgeFormField} style={{ maxWidth: '300px', minWidth: '200px' }}>
                                  ...
                                  <div className={styles.customDropdown} ref={descriptionDropdownRef} style={{ maxWidth: '320px', width: '100%' }}>
                                    <button
                                      ...
                                      className={styles.customDropdownTrigger}
                                      onClick={() => {
                                        const opening = !isDescriptionDropdownOpen;
                                        setIsDescriptionDropdownOpen(opening);
                                        if (opening) {
                                          setDescriptionMode('source');
                                          setLookUpError(null);
                                        }
                                      }}
                                      onFocus={() => setFocusedElement('field')}
                                    ...
                                    {isDescriptionDropdownOpen && (
                                      <div className={styles.descriptionDropdownMenu}>
                                        <div className={styles.descriptionModeToggle}>
                                          <button type="button"
                                            className={`${styles.descriptionModeBtn} ${descriptionMode === 'source' ? styles.descriptionModeBtnActive : ''}`}
                                            onClick={() => setDescriptionMode('source')}>
                                            Project Source
                                          </button>
                                          <button type="button"
                                            className={`${styles.descriptionModeBtn} ${descriptionMode === 'bulletPoints' ? styles.descriptionModeBtnActive : ''}`}
                                            onClick={() => setDescriptionMode('bulletPoints')}>
                                            Bullet Points
                                          </button>
                                        </div>
                                        {descriptionMode === 'source' && (
                                          <div className={styles.projectSourceContainer}>
                                            ...
                                            {lookUpError && <p className={styles.descriptionErrorMsg}>{lookUpError}</p>}
                                          </div>
                                        )}
                                        {descriptionMode === 'bulletPoints' && (
                            ...
                            />
                              ...
                              <div className={styles.knowledgeFormRow}>
                                <div className={styles.knowledgeFormField}>
                                  ...
                                  <div className={`${styles.knowledgeFieldWrapper}${!isCareerFocusSelected ? ` ${styles.disabledFieldWrapper}` : ''}`}>
                          ...
                          <div className={styles.knowledgeFormField}>
                            ...
                            <div className={styles.customDropdown} ref={futureDescriptionDropdownRef} style={{ maxWidth: '320px', width: '100%' }}>
                              <button
                                ...
                                className={styles.customDropdownTrigger}
                                onClick={() => {
                                  const opening = !isFutureDescriptionDropdownOpen;
                                  setIsFutureDescriptionDropdownOpen(opening);
                                  if (opening) {
                                    setFutureDescriptionMode('source');
                                    setFutureLookUpError(null);
                                  }
                                }}
                                onFocus={() => setFocusedElement('field')}
                                aria-label="Edit Project Description"
                                  ...
                                  />
                                      ...
                                      {isFutureDescriptionDropdownOpen && (
                                        <div className={styles.descriptionDropdownMenu}>
                                          <div className={styles.descriptionModeToggle}>
                                            <button type="button"
                                              className={`${styles.descriptionModeBtn} ${futureDescriptionMode === 'source' ? styles.descriptionModeBtnActive : ''}`}
                                              onClick={() => setFutureDescriptionMode('source')}>
                                              Project Source
                                            </button>
                                            <button type="button"
                                              className={`${styles.descriptionModeBtn} ${futureDescriptionMode === 'bulletPoints' ? styles.descriptionModeBtnActive : ''}`}
                                              onClick={() => setFutureDescriptionMode('bulletPoints')}>
                                              Bullet Points
                                            </button>
                                          </div>
                                          {futureDescriptionMode === 'source' && (
                                            <div className={styles.projectSourceContainer}>
                                              <textarea
                                                className={styles.descriptionTextarea}
                                                value={project.projectSource || ''}
                                              ...
                                              {futureLookUpError && <p className={styles.descriptionErrorMsg}>{futureLookUpError}</p>}
                                            </div>
                                          )}
                                          {futureDescriptionMode === 'bulletPoints' && (
                                            <div className={styles.bulletPointsList}>
                                              <div className={styles.bulletPointItem}>
                                                <span className={styles.bulletPointNumber}>•</span>
                                                <textarea className={styles.bulletPointInput} data-bp
...
onClick={() => {
              ...
              {activeSection === 'resume' && (
                <ResumeSection
                  ...
                  onNavigateToAnalysis={(data) => {
                    if (data) {
                      const isFromExisting = data.fromExistingResume || false;
                      ...
                      setAnalysisInitialData({
                        ...
                        fromExistingResume: isFromExisting,
                        // Unique ID per click — lets AnalysisSection distinguish a fresh navigation
                        // from a tab remount with the same job data
              ...
              {activeSection === 'analyzer' && (
                <AnalysisSection
                  ...
                  fromExistingResume={analysisInitialData?.fromExistingResume || false}
                  initialNavigationId={analysisInitialData?.navigationId}
                  onNavigateToProfile={() => setActiveSection('profile')}
                  onNavigateToKnowledge={() => setActiveSection('knowledge')}
      ...
      {showDownloadLimitToast && (
        <div className={styles.craftLimitToast}>
          ...
          <div className={styles.craftLimitToastBody}>
            <p className={styles.craftLimitToastTitle}>Free quota reached</p>
            <p className={styles.craftLimitToastText}>
              You&apos;ve used all 3 free smart downloads. Subscribe as a Pro plan user to unlock unlimited AI features for resume craft, smart download, and personal capability analysis.
            </p>
          </div>
          <button
            className={styles.craftLimitToastClose}
            onClick={() => setShowDownloadLimitToast(false)}
            aria-label="Dismiss"
          >
            ×
          </button>
      ...
      {showOverwriteConfirmModal && (
        <div className={styles.modalOverlay} onClick={() => { setShowOverwriteConfirmModal(false); setPendingAutoFillData(null); }}>
          <div className={styles.modalContent} style={{ maxWidth: '480px' }} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h3 className={styles.modalTitle}>Overwrite Existing Data?</h3>
              <button type="button" className={styles.modalCloseButton} onClick={() => { setShowOverwriteConfirmModal(false); setPendingAutoFillData(null); }} aria-label="Close">
            ...
            <div className={styles.modalBody}>
              <p style={{ color: '#5a5248', marginBottom: '1.5rem', lineHeight: '1.6' }}>
                You already have information filled in your Profile and Knowledge base. Would you like to overwrite it with the data extracted from your resume?
              </p>
              <div style={{ display: 'flex', gap: '1rem', justifyContent: 'flex-end' }}>
                <button type="button" className={styles.backButton} onClick={() => { setShowOverwriteConfirmModal(false); setPendingAutoFillData(null); }}>
                  <span className={styles.backButtonText}>Keep Existing</span>
                </button>
                <button type="button" className={styles.nextButton} onClick={() => { if (pendingAutoFillData) applyAutoFillData(pendingAutoFillData); setShowOverwriteConfirmModal(false); setPendingAutoFillData(null); }}>
                  <span className={styles.nextButtonText}>Overwrite</span>
                </button>
              </div>
            </div>
          </div>
